            ...
            <div class="main_warp">
                <div class="register_warp">
                    ...
                    <div class="register_form">
                        <!--手机号码-->
                        <div class="telephone">
                            <i></i><input type="text" v-model="telephone" @blur="checkTelephone"
                                          @focus="clearTelephoneMsg" placeholder="请输入手机号码" required="required"/>
                        </div>
                        <div :class="{telephone_msg:true,red:telephoneRed}">{{telephoneError}}</div><!--提示信息-->

                        <!--验证码-->
                        <div class="code">
                            <i></i><input type="text" v-model="validateCode" @blur="checkValidate" placeholder="请输入验证码"
                                          required="required">
                        </div>
                        <input type="button" v-bind:value="buttonContext" @click="sendCode"
                               v-bind:class="{getCode:true,ban:!buttonFlag,notBan:buttonFlag}"
                               v-bind:disabled="!buttonFlag" required="required"/>
                        <div :class="{code_msg:true,red:validateCodeRed}">{{validateCodeError}}</div><!--提示信息-->

                        <!--密码-->
                        <div class="password">
                            <i></i><input type="password" v-model="password" @blur="checkPassword"
                                          placeholder="密码以字母开头，6-18位，数字和符号组合"/>
                        </div>
                        <div :class="{password_msg:true,red:passwordRed}">{{ passwordError}}</div><!--提示信息-->

                        <!--重复密码-->
                        <div class="pwd">
                            <i></i><input type="password" v-model="repeatPassword" @blur="checkRepeatPassword"
